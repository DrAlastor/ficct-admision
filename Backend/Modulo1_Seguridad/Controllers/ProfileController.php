...
use Backend\Modulo1_Seguridad\Requests\ProfileUpdateRequest;
...
class ProfileController extends Controller
{
    ...
    public function edit(Request $request): Response
    {
        ...
        if ($usuario->rol_id == 3) {
            $postulante = \Backend\Modulo2_Admision\Models\Postulante::find($perfil->id);
            if ($postulante) {
                $perfil->colegio_procedencia = $postulante->colegio_procedencia;
                $perfil->ciudad = $postulante->ciudad;
            }
        } elseif ($usuario->rol_id == 2) {
            $docente = \Backend\Modulo3_Academico\Models\Docente::find($perfil->id);
            if ($docente) {
        ...
        return Inertia::render('Modulos/Modulo1_Seguridad/Edit', [
